...
def get_colmap_camera_params(colmap_dir, img_files):
    cameras = read_cameras_binary(colmap_dir + "/cameras.bin") # facol, W/2, H/2
    images = read_images_binary(colmap_dir + "/images.bin") # 48
    colmap_image_idcs = {v.name: k for k, v in images.items()} # 48
    # num_train = int(len(images)/2)
    colmap_image_idcs = {}
    for k, v in images.items():
        if "_left.jpg" in v.name:
            i = int(v.name.split('_')[0])
            key = "%05d"%(2*i)+".png"
        elif "_right.jpg" in v.name:
            i = int(v.name.split('_')[0])
            key = "%05d"%(2*i+1)+".png"
        else:
            key = v.name
        colmap_image_idcs[key] = k
    ...
    for idx, name in enumerate(img_names): # idx: 0, 1, 2, ... 
        key = colmap_image_idcs[name] # key: 1, 2, 3, ...
        image = images[key]
        # assert (int(image.name.split('_')[0]) == (int(int(name.split('.')[0])/2) if ("_left.jpg" in image.name) else int((int(name.split('.')[0])-1)/2))) \
        #     or (image.name == name)
        K, extrinsics = get_intrinsics_extrinsics(image, cameras)
        K_all[idx] = K # 4, 4
        extrinsics_all[idx] = extrinsics # 4, 4
