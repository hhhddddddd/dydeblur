...
def solve_procrustes(
    ...
    weights = weights[:, None] / weights.sum() # (N, 1)
    # Normalize point positions.
    src_mean = (src * weights).sum(dim=0) # (3,)
    dst_mean = (dst * weights).sum(dim=0) # (3,)
    src_cent = src - src_mean # (N, 3)
    dst_cent = dst - dst_mean # (N, 3)
    # Normalize point scales.
    if not enforce_se3:
        src_scale = (src_cent**2 * weights).sum(dim=-1).mean().sqrt()
        dst_scale = (dst_cent**2 * weights).sum(dim=-1).mean().sqrt()
    else:
        src_scale = dst_scale = src.new_tensor(1.0) # 1.0
    src_scaled = src_cent / src_scale # (N, 3)
    dst_scaled = dst_cent / dst_scale # (N, 3)
    # Compute the matrix for the singular value decomposition (SVD).
    matrix = (weights * dst_scaled).T @ src_scaled # (3, 3)
...
def get_weights_for_procrustes(clusters, visibilities=None): # clusters: [num_frames, n_pts, 3], visibilities: # [num_frames, n_pts]
    clusters_median = clusters.median(dim=-2, keepdim=True)[0] # [num_frames, 1, 3]
    dists2clusters_center = torch.norm(clusters - clusters_median, dim=-1) # [num_frames, n_pts]
    dists2clusters_center /= dists2clusters_center.median(dim=-1, keepdim=True)[0] # normalize
    weights = torch.exp(-dists2clusters_center) # [num_frames, n_pts]
