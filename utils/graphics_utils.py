...
def getWorld2View2(R, t, translate=np.array([.0, .0, .0]), scale=1.0): # world -> camera
    ...
    cam_center = C2W[:3, 3]
    cam_center = (cam_center + translate) * scale # camera center translation and scale
    C2W[:3, 3] = cam_center
    Rt = np.linalg.inv(C2W)
    return np.float32(Rt)


def getProjectionMatrix(znear, zfar, fovX, fovY): # camera -> ndc
    ...
    P[3, 2] = z_sign # -1 ?
    P[2, 2] = z_sign * zfar / (zfar - znear)    # A
    P[2, 3] = -(zfar * znear) / (zfar - znear)  # B
    return P
