...
def init_dme():
    vgg = VGG19_down()
    unet = UNet_up()
    W = np.load("/home/xuankai/code/d-3dgs/pretrained/dmenet_pretrained.npz")
    vgg_sd = {}
    unet_sd = {}
    for name in W.files:
        sp = name.split('/')
...
def build_focus_map(vgg, unet, image_path, save_path, map_path):

    for image in sorted(os.listdir(image_path)):

        imgs = torch.from_numpy(np.array(Image.open(os.path.join(image_path, image)))) / 255 # 360, 940, 3
        imgs = imgs.permute(2,0,1) # 3, 360, 940
        imgs = imgs[None] # 1, 3, 360, 940
        
        # if imgs.shape[2] == 360:
        #     imgs = F.pad(imgs, (2,2,4,4), "reflect") # 1, 3, 368, 944
        # else:
        #     imgs = F.pad(imgs, (2,2,0,0), "reflect") # 1, 3, 400, 944
       
        feat = vgg(imgs) # 0: 1, 64, 368, 944;  1: 1, 128, 184, 472;  2: 1, 256, 92, 236;  3: 1, 512, 46, 118;  4: 1, 512, 23, 59
        focus_map = unet(feat) # 1, 1, 368, 944
        
        # if imgs.shape[2] == 368:
        #     focus_map = focus_map[..., 4:-4, 2:-2] # 1, 1, 360, 940
        # else:
        #     focus_map = focus_map[..., 2:-2] # 1, 1, 360, 940

        focus_map = focus_map.squeeze() # 360, 940

        image_save = (focus_map.detach()-focus_map.detach().min())/(focus_map.detach().max()-focus_map.detach().min())
        cv2.imwrite(os.path.join(map_path, image), image_save.numpy()*255)
        np.save(os.path.join(save_path, image.split('.')[0]+".npy"), focus_map.detach().numpy())
...
if __name__ == "__main__":
    ...
    for scene in sorted(os.listdir(path)):
        # image_path = path + scene + "/images_2"
        # save_path = path + scene + "/blur_masks_npy"
        # map_path = path + scene + "/blur_masks"
        image_path = path + scene + "/dense/images_512x288"
        save_path = path + scene + "/dense/blur_masks_npy"
        map_path = path + scene + "/dense/blur_masks"
        os.makedirs(save_path, exist_ok=True)
        os.makedirs(map_path, exist_ok=True)
        build_focus_map(vgg, unet, image_path, save_path, map_path)
        print(scene, "is ok!")

    print("Foucus Map is OK!")
